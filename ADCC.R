...
uspec = multispec(replicate(11, xspec))
spec1a = dccspec(uspec = uspec, dccOrder = c(1, 1), model='aDCC', distribution = 'mvnorm')
cl = makePSOCKcluster(11)
multf = multifit(uspec, data = data.frame(OVXR,GVZR,VXSLVR,ECOR,S.PGCER,SolarR,S.P.TSX.RECTR,FTSE4GOOD.UK.50R,FTSE4GOOD.US.100R,FTSE4GOOD.Europe.50R,FTSE4GOOD.US.Global.100R), cluster = cl)
fit1a = dccfit(spec1a, data = data.frame(OVXR,GVZR,VXSLVR,ECOR,S.PGCER,SolarR,S.P.TSX.RECTR,FTSE4GOOD.UK.50R,FTSE4GOOD.US.100R,FTSE4GOOD.Europe.50R,FTSE4GOOD.US.Global.100R), fit.control = list(eval.se = TRUE), fit = multf, cluster = cl)
fit1a
correlation = rcor(fit1a)
correlation
dim(correlation)
summary(correlation)
